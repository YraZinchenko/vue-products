    ...
    <layout-default>
      <div class="main">
        <div class="sidebar">
          ...
          >
            {{category}}
            // во всех пременных во всем проэкте хорошей практикой считается ставить пробелы в переменных, так улучшается читка кода {{ category }}
          </div>
          <div>
  ...
  created () {
    getProducts()
      .then((response) => {
        ...
        // Как так выходит что с response.data мы получаем и продукты и фильтр продукты?
        ...
        // вот здесь скорее всего ненужная лишняя логика, зачем еще from new set непонятно
